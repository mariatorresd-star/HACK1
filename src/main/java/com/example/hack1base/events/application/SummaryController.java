...
public class SummaryController {
    ...
    public ResponseEntity<Map<String, Object>> generateWeeklyPremiumSummary(@RequestBody SummaryRequest request,
                                                                            Authentication auth) {

        if (request.getEmailTo() == null || request.getEmailTo().isEmpty()) {
            return ResponseEntity.badRequest().body(Map.of(
                    "error", "BAD_REQUEST",
                    "message", "El campo emailTo es obligatorio"
            ));
        }
        ...
        if (auth != null && auth.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_BRANCH"))) {
            if (request.getBranch() == null || request.getBranch().isBlank()) {
                return ResponseEntity.status(400).body(Map.of(
                        "error", "BAD_REQUEST",
                        "message", "Branch es obligatorio para usuarios BRANCH"
                ));
            }
            Object principal = auth.getPrincipal();
            if (principal instanceof Account acc) {
                ...
                if (userBranch != null && !request.getBranch().equalsIgnoreCase(userBranch)) {
                    return ResponseEntity.status(403).body(Map.of(
                            "error", "FORBIDDEN",
                            "message", "No puede solicitar resúmenes de otra sucursal"
                    ));
                }
            }
        }
        ...
        publisher.publishEvent(new ReportRequestedEvent(
                this, request, requestId, auth.getName(), true, true, true
        ));
